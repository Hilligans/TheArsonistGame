...
import dev.hilligans.engine2d.client.tasks.DeadScreen;
...
public class Engine2D implements ModClass {
    ...
    public void registerContent(ModContainer container) {
        ...
        if(container.getGameInstance().getSide().isClient()) {
            ...
            container.registerRenderTask(new DeadScreen());
            ...
            container.registerRenderTarget(new RenderTarget("death_screen_renderer", "engine2D:pipeline2d", "engine2D:dead_screen")
            ...
            container.registerKeybinds(new RepeatingInput("move_up", "ourcraft:key_press_handler::" + GLFW_KEY_W,
                    (window, strength) -> ((Client2D) window.getClient()).setVelY(-MOVE_SPEED)).onlyWithPipelines("engine2D:pipeline2d"));

            container.registerKeybinds(new RepeatingInput("move_left", "ourcraft:key_press_handler::" + GLFW_KEY_A,
                    (window, strength) -> ((Client2D) window.getClient()).setVelX(-MOVE_SPEED)).onlyWithPipelines("engine2D:pipeline2d"));
